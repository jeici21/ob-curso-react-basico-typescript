const AsyncExample = () => {
    ...
    async function saveSessionStorage(key: string, value: string) {
        sessionStorage.setItem(key, value)
        return Promise.resolve(sessionStorage.getItem(key))
    }

    function showStorage() {
        saveSessionStorage('name', 'Martín').then(response => {
            let value = response
    ...
    const returnError = async () => await Promise.reject(new Error('Oooops!'))
    ...
    const multiplePromise = async () => {
        let results = await Promise.all(
            [fetch('https://regres.in/api/users'), fetch('https://regres.in/api/users?page=2')]
